...
// 1º Filtramos los que juegan a algo que incluya 'Legends'
// 2º Mapeamos el resultado para transformar a mayúsculas si tienen > 35 años
const legendsStreamers = streamers3
    .filter(streamer => streamer.gameMorePlayed.includes('Legends'))
    .map(streamer => {
        if (streamer.age > 35) {
            // Hacemos una copia del objeto y sobreescribimos la propiedad
            return {...streamer, gameMorePlayed: streamer.gameMorePlayed.toUpperCase()};
        }
        return streamer; // Si tiene 35 o menos, lo devolvemos tal cual
    });

console.log("4.5 - Streamers de Legends (mayúsculas si > 35):", legendsStreamers);